...
try {
  rawInputData = fs.readFileSync(`${__dirname}/demo2.txt`, 'utf8')
} catch (e) {
  console.log(`Error!`)
  console.error(e)
}
  ...
  inputData.forEach(refMove => {
    ...
    while (iteration !== (xMove || yMove)) {
      ...
      body.forEach((bodyUnit, bodyIndex) => {
        let [lastXCoord, lastYCoord] = !!bodyIndex ? newBody.at(bodyIndex - 1) : [xHeadCoord, yHeadCoord]

        let knotGaps = bodyUnit
          .map(
